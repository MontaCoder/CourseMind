...
const ProfilePricing = () => {
  ...
  const getCurrentPlanId = () => {
    ...
    if (type === MonthType || type === 'monthly') return 'monthly';
    if (type === YearType || type === 'yearly') return 'yearly';
    return 'free';
  };
          ...
          >
            ...
            <CardFooter>
              <Button
                ...
                disabled={currentPlanId === 'forever' || currentPlanId === plan.id}
                className={`w-full ${plan.isPopular ? 'bg-primary hover:bg-primary/90' : ''}`}
                variant={plan.isPopular ? "default" : "outline"}
              >
                {currentPlanId === 'forever' ? 'Lifetime Access' : currentPlanId === plan.id ? 'Current Plan' : plan.id === 'free' ? 'Manage Plan' : 'Change Plan'}
              </Button>
            </CardFooter>
          </Card>
        ))}
