  ...
  useEffect(() => {
    async function dashboardData() {
      ...
      try {
        ...
        const processedData = posts.map((post: BlogPost) => ({
          ...post,
          imageUrl: post.imageUrl || getImage(post.image)
        }));
        ...
        if (featuredBlog) {
          ...
          setFeatured(processedData[0] || null); // Set latest blog as featured if none are featured
        }
        ...
        if (popularBlogs.length > 0) {
          ...
          setPopular(processedData[0] ? [processedData[0]] : []); // Set latest blog as popular if none are popular
        }
      } catch (error) {
        console.error(error);
        setData([]);
        ...
        <div className="container max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="mb-16">
            {isLoading ?
              ...
              :
              featured ? <Card className="overflow-hidden bg-card">
                <div className="md:grid md:grid-cols-2">
                  ...
                  <div className="p-6 md:p-8 flex flex-col">
                    ...
                    <Button onClick={() => readMore(featured._id, featured.category, featured.date, featured.excerpt, featured.imageUrl || '/placeholder.svg', featured.title, featured.tags, featured.content)} className="self-start">
                      Read Article
                      <ArrowRight className="ml-2 h-4 w-4" />
                    </Button>
                  </div>
                </div>
              </Card> : <Card className="p-8 text-center"><p className="text-muted-foreground">No blog posts have been published yet.</p></Card>
            }
          </div>
                  ...
                  (
                    data.map((post) => (
                      <Card key={post._id} className="flex flex-col h-full">
                        ...
                        <CardFooter className="pt-0">
                          <Button onClick={() => readMore(post._id, post.category, post.date, post.excerpt, post.imageUrl || '/placeholder.svg', post.title, post.tags, post.content)} variant="ghost" className="p-0 h-auto">
                            Read More
                  ...
                  <div className="space-y-4">
                    {popular.map((post) => (
                      <div key={`popular-${post._id}`} className="flex gap-3">
                        ...
                        <div>
                          <h4 onClick={() => readMore(post._id, post.category, post.date, post.excerpt, post.imageUrl || '/placeholder.svg', post.title, post.tags, post.content)} className="font-medium text-sm line-clamp-2 cursor-pointer">
                            {post.title}
                          </h4>
